...
/**
 * @title CoinEstate Token Sales - Phase 1: Direct Sales
 * @dev Optimized for direct CET sales on homepage with automated monthly distributions
 */
contract CoinEstateSales is Initializable,
    ...
    uint256 public constant CET_PRICE_EUR = 1 ether; // €1.00 per CET
    ...
    // Events
    event CETTokenPurchased(address indexed buyer, uint256 amount, uint256 price);
    ...
    function initialize(
        ...
        __ERC20_init("CoinEstate Token", "CET");
    ...
    /**
     * @dev Purchase CET tokens directly (for homepage integration)
     * @param amount Amount of CET tokens to purchase
     */
    function purchaseCET(uint256 amount) external payable nonReentrant whenNotPaused {
        require(salesEnabled, "Sales disabled");
        require(amount > 0, "Amount must be positive");
        require(amount <= balanceOf(treasuryWallet), "Insufficient treasury balance");
        
        uint256 cost = amount; // 1 CET = €1 = 1 ETH equivalent for simplicity
        require(msg.value >= cost, "Insufficient payment");
        
        // Transfer CET from treasury to buyer
        ...
        emit CETTokenPurchased(msg.sender, amount, cost);
    ...
    /**
     * @dev Purchase CET with stable tokens (alternative payment method)
     */
    function purchaseCETWithStable(uint256 amount) external nonReentrant whenNotPaused {
        require(salesEnabled, "Sales disabled");
        require(amount > 0, "Amount must be positive");
        require(amount <= balanceOf(treasuryWallet), "Insufficient treasury balance");
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount) external onlyOwner {
        ...
        require(tokenAddress != address(this), "Cannot recover CET");
